...
async function debugLocumTrueRate() {
  let browser;
  try {
    console.log('🚀 Starting LocumTrueRate validation session...');
    ...
    try {
      ...
      const cssInfo = await page.evaluate(() => {
        ...
        return {
          ...
          stylesheetSources: stylesheets.map(sheet => {
            try {
              return sheet.href || 'inline';
            } catch (e) {
              return 'cross-origin or blocked';
            }
          })
        };
      });
      ...
      console.log('[HOMEPAGE CONTENT]', content ? 'Content loaded' : 'Homepage appears to be blank');
      ...
      const hasStyling = cssInfo.stylesheetCount > 0 && (cssInfo.bodyBackgroundColor !== 'rgba(0, 0, 0, 0)' || cssInfo.hasTailwindClasses);
      console.log(`[CSS STYLING] ${hasStyling ? '✅ CSS loaded and applied' : '❌ No CSS styling detected'}`);
