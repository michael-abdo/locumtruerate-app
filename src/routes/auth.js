...
router.get('/verify', requireAuth, async (req, res) => {
  try {
    config.logger.info(`Token verification for user: ${req.user.id}`, 'AUTH_VERIFY');
    ...
    if (!user) {
      return createErrorResponse(res, 401, 'User not found', 'user_not_found');
    }
    ...
    config.logger.error('Token verification error', error, 'AUTH_VERIFY');
    return createErrorResponse(res, 500, 'Internal server error during verification', 'verification_failed');
  }
});
...
router.post('/refresh', requireAuth, async (req, res) => {
  try {
    config.logger.info(`Token refresh for user: ${req.user.id}`, 'AUTH_REFRESH');
    ...
    config.logger.error('Token refresh error', error, 'AUTH_REFRESH');
    return createErrorResponse(res, 500, 'Internal server error during token refresh', 'refresh_failed');
  }
});
