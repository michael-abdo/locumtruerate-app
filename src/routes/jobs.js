...
router.get('/search', async (req, res) => {
  try {
    ...
    if (!q || q.trim().length === 0) {
      return createErrorResponse(res, 400, 'Search query is required', 'missing_query');
    }
    
    config.logger.info(`Job search request: ${q}`, 'JOB_SEARCH');
    
    // Use the existing findAll method with search query
    ...
    config.logger.info(`Search returned ${result.jobs.length} jobs for query: ${q}`, 'JOB_SEARCH');
    ...
    config.logger.error('Job search error', error, 'JOB_SEARCH');
    return createErrorResponse(res, 500, 'Internal server error while searching jobs', 'job_search_failed');
  }
});
