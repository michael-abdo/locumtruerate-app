...
// Root redirect to index.html (handled by static middleware)
app.get('/', (req, res) => {
  config.logger.info('Root access - serving index.html');
...
app.get(`/api/${API_VERSION}`, (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      calculationStats: `/api/${API_VERSION}/calculations/stats`,
      duplicateCalculation: `/api/${API_VERSION}/calculations/:id/duplicate`,
      favoriteCalculation: `/api/${API_VERSION}/calculations/:id/favorite`,
      archiveCalculation: `/api/${API_VERSION}/calculations/:id/archive`,
...
app.use(`/api/${API_VERSION}/calculations`, calculationsRoutes);
...
app.use((req, res) => {
  config.logger.warn(`404 Not Found: ${req.method} ${req.path}`, 'SERVER');
  
  // For HTML requests, serve the index.html (SPA fallback)
  if (req.accepts('html')) {
