...
/**
 * Wrapper for createSuccessResponse to match calculations route usage
 * @param {Object} res - Express response object
 * @param {Object} payload - Response payload with message and data
 * @param {number} status - HTTP status code (default: 200)
 * @returns {Object} JSON success response
 */
const sendSuccessResponse = (res, payload, status = 200) => {
  const { message, data, meta } = payload;
  return createSuccessResponse(res, status, data, message, meta);
};
